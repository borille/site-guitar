...
class My_Controller_Action_Helper_ServerHelper extends Zend_Controller_Action_Helper_Abstract
{
    ...
    function getUsoHD()
    {
        ...
        if ( strtoupper( substr( PHP_OS, 0, 3 ) ) === 'WIN' )
        {
            ...
            $saida[2]['size'] = My_Action_Helper::resizeMem( $dts );
            $saida[2]['used'] = My_Action_Helper::resizeMem( $dts - $dfs );
            $saida[2]['avail'] = My_Action_Helper::resizeMem( $dfs );
    ...
    function getUsoRam()
    {
        ...
        if ( strtoupper( substr( PHP_OS, 0, 3 ) ) === 'WIN' )
        {
            ...
            $saida[$i]['total'] = 0;
            $saida[$i]['used'] = My_Action_Helper::resizeMem( memory_get_usage( true ) );
            $saida[$i]['percent'] = 0;
            $saida[$i]['free'] = 0;
            $saida[$i]['shared'] = 0;
            $saida[$i]['buffers'] = 0;
                ...
                foreach ( $retval as $value )
                {
                    if ( $value )
                    {
                        ...
                        if ( intval( $tmp_arr[1] ) )
                        {
                            ...
                            $saida[$i]['total'] = My_Action_Helper::resizeMem( $tmp_arr[1] );
                            $saida[$i]['used'] = My_Action_Helper::resizeMem( $tmp_arr[2] );
                            $saida[$i]['percent'] = intval( $tmp_arr[2] * 100 / $tmp_arr[1] );
                            $saida[$i]['free'] = My_Action_Helper::resizeMem( $tmp_arr[3] );
                            $saida[$i]['shared'] = My_Action_Helper::resizeMem( $tmp_arr[4] );
                            $saida[$i]['buffers'] = My_Action_Helper::resizeMem( $tmp_arr[5] );
                            $saida[$i]['cached'] = My_Action_Helper::resizeMem( $tmp_arr[6] );
